...
pub fn add_seed(args: AddSeedArgs, overwrite: bool) -> CommandResult {
    let root = match find_project_root() {
        Ok(r) => r,
        Err(e) => return CommandResult::err("add:seed", e.to_string()),
    };
    ...
    let template = match engine.get_template("features/seed.jinja") {
        Ok(t) => t,
        Err(e) => return CommandResult::err("add:seed", format!("Template error: {}", e)),
    };
    ...
    let rendered = match template.render(minijinja::context!(
        ...
        Ok(r) => r,
        Err(e) => return CommandResult::err("add:seed", format!("Render error: {}", e)),
    };

    let output_path = resolve_output_path(&root, &format!("db/seeds/{}.ts", args.name));

    let outcome = match write_file(&output_path, &rendered, overwrite) {
        Ok(o) => o,
        Err(e) => return CommandResult::err("add:seed", format!("Write error: {}", e)),
    };
    ...
    CommandResult::ok("add:seed", files_created)
}
